...
Современный стандарт *ESM* позволяет применять оператор `await` на самом
верхнем уровне модуля. Стандарт *CommonJS* не поддерживает такую возможность.
Использование оператора `await` в корне файла *CommonJS* приводит
